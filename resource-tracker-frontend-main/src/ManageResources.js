...
function ManageResources() {
  ...
  useEffect(() => {
    const fetchResources = async () => {
      try {
        if (permissionid === "1") {
          const res = await axios.get(`https://api.i-raysolutions.com/api/v1/resource/list`);
          ...
          const res = await axios.get(
            `https://api.i-raysolutions.com/api/v1/resource/getAllResourcesByManagerId/${id}`
          );
          const activeResources = res.data.filter(
            (resource) =>
              resource.status?.toLowerCase() !== "terminated" &&
          ...
          const res = await axios.get(`https://api.i-raysolutions.com/api/v1/resource/${id}`);
  ...
  const deleteRes = async (resourceId) => {
    try {
      ...
      const deleteResponse = await axios.delete(
        `https://api.i-raysolutions.com/api/v1/resource/${resourceId}`
      );
      if (deleteResponse.status === 200) {
        if (permissionid === "1") {
          const response = await axios.get(`https://api.i-raysolutions.com/api/v1/resource/list`);
          ...
          const response = await axios.get(`https://api.i-raysolutions.com/api/v1/resource/${id}`);
