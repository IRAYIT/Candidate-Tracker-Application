...
function ManageResources() {
  ...
  useEffect(() => {
    const fetchResources = async () => {
      try {
        if (permissionid === "1") {
          const res = await axios.get(`https://candidate-tracker-iray-bhgbahe3bjame4gv.centralindia-01.azurewebsites.net/api/v1/resource/list`);
          ...
          const res = await axios.get(
            `https://candidate-tracker-iray-bhgbahe3bjame4gv.centralindia-01.azurewebsites.net/api/v1/resource/getAllResourcesByManagerId/${id}`
          );
          const activeResources = res.data.filter(
            (resource) =>
              resource.status?.toLowerCase() !== "terminated" &&
          ...
          const res = await axios.get(`https://candidate-tracker-iray-bhgbahe3bjame4gv.centralindia-01.azurewebsites.net/api/v1/resource/${id}`);
  ...
  const deleteRes = async (resourceId) => {
    try {
      ...
      const deleteResponse = await axios.delete(
        `https://candidate-tracker-iray-bhgbahe3bjame4gv.centralindia-01.azurewebsites.net/api/v1/resource/${resourceId}`
      );
      if (deleteResponse.status === 200) {
        if (permissionid === "1") {
          const response = await axios.get(`https://candidate-tracker-iray-bhgbahe3bjame4gv.centralindia-01.azurewebsites.net/api/v1/resource/list`);
          ...
          const response = await axios.get(`https://candidate-tracker-iray-bhgbahe3bjame4gv.centralindia-01.azurewebsites.net/api/v1/resource/${id}`);
                            ...
                            {permissionid === "1" && (
                              <button
                                onClick={() => {
                                  ...
                                  localStorage.setItem("temp_id_for_use", `${row.original.id}`);
                                }}
                                className="px-3 py-1 rounded border text-gray-700 hover:bg-gray-100 text-xs transition cursor-pointer"
                              >
                                Edit
