      ...
      try {
        if (permissionid === "1") {
          ...
          const res = await axios.get(
            `https://candiate-tracker-aea8hqfwbxd4dqhu.centralindia-01.azurewebsites.netapi/v1/resource/getAllResourcesByManagerId/${id}`
          );
          const activeResources = res.data.filter(
            (resource) =>
              resource.status?.toLowerCase() !== "terminated" &&
          ...
          const res = await axios.get(`https://candiate-tracker-aea8hqfwbxd4dqhu.centralindia-01.azurewebsites.netapi/v1/resource/${id}`);
  ...
  const deleteRes = async (resourceId) => {
    try {
      ...
      const deleteResponse = await axios.delete(
        `https://candiate-tracker-aea8hqfwbxd4dqhu.centralindia-01.azurewebsites.netapi/v1/resource/${resourceId}`
      );
      if (deleteResponse.status === 200) {
        if (permissionid === "1") {
          ...
          const response = await axios.get(`https://candiate-tracker-aea8hqfwbxd4dqhu.centralindia-01.azurewebsites.netapi/v1/resource/${id}`);
