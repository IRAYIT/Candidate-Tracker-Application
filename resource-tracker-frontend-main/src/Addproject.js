...
function RoleCard({ role, index, isFirst, onUpdate, onRemove, roleError }) {
  ...
  const fetchResources = useCallback(async (tech, customTech, skills) => {
    ...
    try {
      const res = await axios.post("https://api.i-raysolutions.com/api/v1/projects/getResourceNames", {
        permissionId: 3,
...
function Addproject() {
  ...
  const handleSubmit = () => {
    ...
    axios.post("https://api.i-raysolutions.com/api/v1/projects/createproject", payload)
