...
const getAllOpenings = async (pid = permissionid) => {
  const url = pid === "1"
    ? `https://candiate-tracker-aea8hqfwbxd4dqhu.centralindia-01.azurewebsites.net/api/v1/openings/all`
    : `https://candiate-tracker-aea8hqfwbxd4dqhu.centralindia-01.azurewebsites.net/api/v1/openings`;
  ...
  const deleteopening = (openingId) => {
    setLoading(true);
    axios
      .delete(`https://candiate-tracker-aea8hqfwbxd4dqhu.centralindia-01.azurewebsites.net/api/v1/openings/${openingId}`)
      .then((res) => {
        if (res.status === 200) {
          getAllOpenings();
          setLoading(false);
